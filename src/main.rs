...
fn logic() {
    ...
    for row in 0..H {
        ...
        let remaining_tokens = total_tokens - current_token_idx;
        
        let tokens_for_this_line = if remaining_rows == 1 {
            remaining_tokens
        } else {
            let mut count = 0;
            let mut width = 0;
            while current_token_idx + count < total_tokens {
                let t_len = tokens[current_token_idx + count].chars().count();
                let space = if count == 0 { 0 } else { 1 };
                if width + space + t_len > W {
                    break;
                }
                width += space + t_len;
                count += 1;
                
                let tokens_left = total_tokens - (current_token_idx + count);
                if tokens_left < (remaining_rows - 1) {
                    break;
                }
            }
            count.max(1)
        ...
        for _ in 0..tokens_for_this_line {
    ...
    for row in 0..H {
        ...
        if line_tokens.is_empty() {
            line_str.push_str(&SPACE.to_string().repeat(W));
        } else {
            let content_len: usize = line_tokens.iter().map(|t| t.chars().count()).sum();
            let total_spaces_needed = W - content_len;
            ...
            if gaps > 0 {
                ...
                for (idx, t) in line_tokens.iter().enumerate() {
                    line_str.push_str(t);
                    if idx < gaps {
                        let target = (total_spaces_needed * (idx + 1)) / gaps;
                        let count = target - distributed;
                        line_str.push_str(&SPACE.to_string().repeat(count));
                        distributed += count;
                    }
                }
            } else {
                line_str.push_str(&line_tokens[0]);
                while line_str.chars().count() < W {
                    line_str.push(SPACE);
                }
            }
        }
        
        final_output.push_str(&line_str.chars().take(W).collect::<String>());
        if row < H - 1 {
